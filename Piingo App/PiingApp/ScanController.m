...
@interface ScanController () <AVCaptureMetadataOutputObjectsDelegate, UITextFieldDelegate>
{
    AppDelegate *appDel;
...
-(IBAction)receiveBagClicked:(id)sender {
    ...
    if ([self.delegate respondsToSelector:@selector(didScanCompleteWithBagNo:)])
    {
        ...
        [self dismissViewControllerAnimated:YES completion:^{
            ...
            [self.delegate didScanCompleteWithBagNo:self.scannedBarcode.text];
...
-(void) checkBagFound
{
    ...
    for (int i = 0; i < [self.arrayTotalBags count]; i++)
    {
        ...
        if ([[dict objectForKey:@"BagNo"] isEqualToString:self.scannedBarcode.text])
        {
            if (![arrayScannedBags containsObject:self.scannedBarcode.text])
            {
                [arrayScannedBags addObject:self.scannedBarcode.text];
